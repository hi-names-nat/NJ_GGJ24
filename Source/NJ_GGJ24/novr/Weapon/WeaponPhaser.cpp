...
bool AWeaponPhaser::Fire()
{
	...
	FHitResult Out_Hit;
	const FVector TargetLocation = SpawnLocation + (Player->GetCamera()->GetForwardVector() * MaxRange);
	...
	if (world->LineTraceSingleByChannel(Out_Hit, SpawnLocation, TargetLocation, ECC_GameTraceChannel2))
	{
		...
		{
			healthComponent->TakeDamage(RayDamage);
		}
		else
		{
			if (Out_Hit.Component->IsSimulatingPhysics())
			{
				Out_Hit.Component->AddImpulseAtLocation(TargetLocation-SpawnLocation * 25.F, Out_Hit.Location);
			}
		}
	}
	return true;
